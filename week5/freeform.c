...
int main(int argc, char **argv)
{
       ...
       double* acceleration = (double*) malloc(points * sizeof(double));
       initialise_vector(velocity, points, 0.0);
       initialise_vector(acceleration, points, 0.0);
...
void update_positions(double* positions, int points, double time)
{
        ...
        double m = 1.0;   // mass
        double dt = 1.0 / 25.0; // time step, matches samples per cycle

        // static vectors to store persistent velocity and acceleration
        static double* velocity = NULL;
        static double* acceleration = NULL;

        // allocate on first call
        if (velocity == NULL) {
                velocity = (double*) malloc(points * sizeof(double));
                acceleration = (double*) malloc(points * sizeof(double));
                initialise_vector(velocity, points, 0.0);
                initialise_vector(acceleration, points, 0.0);
        }

        // apply driving force to the first point
        positions[0] = driver(time);
        velocity[0] = 0.0;
        acceleration[0] = 0.0;

        // compute acceleration using Hooke's law
        for (int i = 1; i < points - 1; i++) {
                double left_spring = positions[i - 1] - positions[i];
                double right_spring = positions[i + 1] - positions[i];
                double force = k * (left_spring + right_spring);
                acceleration[i] = force / m;
        }

        // handle last point (free end, no force from the right)
        acceleration[points - 1] = k * (positions[points - 2] - positions[points - 1]) / m;
