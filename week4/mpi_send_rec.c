...
int main(int argc, char **argv)
{
    ...
    // initialises the vector on the root process and sends it to other processes
    if (rank == 0)
    {
    ...
    // Root process sends the vector to all other processes using MPI_Send
    if (rank == 0)
    {
        for (int i = 1; i < num_proc; i++)
        {
            MPI_Send(my_vector, num_arg, MPI_INT, i, 0, MPI_COMM_WORLD);
        }
    }
    else
    {
        // All other processes receive the vector using MPI_Recv
        MPI_Recv(my_vector, num_arg, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    // Print runtime for the communication (MPI_Send and MPI_Recv)
    if (rank == 0)
    {
        printf("Runtime for MPI_Send and MPI_Recv: %lf seconds.\n", runtime);
    }
