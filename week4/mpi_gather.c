...
int main(int argc, char **argv)
{
    ...
    int *my_vector, *all_vectors, my_sum = 0, final_sum = 0, remainder;
    ...
    // Now use MPI_Gather to collect the local sums at the root process
    if (rank == 0)
    {
        int *all_sums = malloc(num_proc * sizeof(int));
        ...
        // Free the all_sums array
        free(all_sums);
    }
    else
    {
        // Non-root processes only send their local sum
        MPI_Gather(&my_sum, 1, MPI_INT, NULL, 1, MPI_INT, 0, MPI_COMM_WORLD);
    }

    // Print runtime for the communication (MPI_Send and MPI_Recv)
    if (rank == 0)
    {
        printf("Runtime for MPI_Gather: %lf seconds.\n", runtime);
    }
